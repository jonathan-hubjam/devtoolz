...
import { Copy, Trash2, FileJson, FileCode, AlertCircle, CheckCircle2, ArrowRightLeft, ArrowRight, ShieldCheck, Hash, Clock, Link2, Fingerprint, Clipboard, Search, KeyRound, Zap, GitCompare, CaseSensitive, CalendarClock, Database, Table2, Binary, Palette } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="border-b border-slate-800/50 bg-[#0B1120] py-5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-sm text-muted-foreground">Convert between JSON and YAML instantly. Edit either side and the other updates in real time with syntax validation.</p>
        ...
        <div className="mt-12 space-y-6">
          ...
          <div className="bg-amber-50/70 border border-amber-200/70 rounded-xl p-6">
            ...
            <div className="space-y-5">
                <div>
                  <p className="text-sm font-semibold text-foreground mb-2">Are JSON and YAML fully interchangeable?</p>
                  <p className="text-sm text-muted-foreground leading-relaxed">Nearly. Every valid JSON document is also valid YAML, but YAML supports features JSON does not — such as comments, anchors, and aliases. Comments are lost when converting YAML to JSON. Multi-document YAML files (separated by <code className="bg-muted text-foreground/90 px-1 py-0.5 rounded font-mono text-xs">---</code>) also cannot be represented as a single JSON document.</p>
                </div>
                <div>
                  <p className="text-sm font-semibold text-foreground mb-2">Why does YAML use indentation instead of braces?</p>
                  <p className="text-sm text-muted-foreground leading-relaxed">YAML was designed for human readability. Indentation removes the visual noise of braces and quotes, making configuration files easier to write and review. The trade-off is that indentation errors are a common source of bugs.</p>
                </div>
                <div>
                  <p className="text-sm font-semibold text-foreground mb-2">Which format should I use for config files?</p>
                  <p className="text-sm text-muted-foreground leading-relaxed">YAML is the standard for Kubernetes, Docker Compose, GitHub Actions, and most DevOps tooling. JSON is better for API payloads, web storage, and situations where comments are not needed and strict parsing is important.</p>
                </div>
              </div>
          </div>
        </div>
