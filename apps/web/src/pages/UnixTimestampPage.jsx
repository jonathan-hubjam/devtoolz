...
import { Copy, Clock, Calendar, ArrowRightLeft, ArrowRight, CheckCircle2, FileJson, Hash, ShieldCheck, Zap, Clipboard, Link2, Fingerprint, FileCode, Search, KeyRound, GitCompare, CaseSensitive, CalendarClock, Database, Table2, Binary, Palette } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="border-b border-slate-800/50 bg-[#0B1120] py-5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-sm text-muted-foreground">Convert Unix timestamps to human-readable dates and back. Auto-detects seconds and milliseconds.</p>
          <div className="flex flex-wrap justify-center gap-4 mt-2">
            <span className="flex items-center gap-1.5 text-xs text-muted-foreground"><CheckCircle2 className="w-3.5 h-3.5 text-blue-400 shrink-0" />Auto-detects seconds & ms</span>
            <span className="flex items-center gap-1.5 text-xs text-muted-foreground"><CheckCircle2 className="w-3.5 h-3.5 text-blue-400 shrink-0" />UTC & local time</span>
        ...
        <div className="mt-12 space-y-6">
          ...
          <div className="bg-amber-50/70 border border-amber-200/70 rounded-xl p-6">
            ...
            <div className="space-y-5">
                <div>
                  <p className="text-sm font-semibold text-foreground mb-2">What happens after the year 2038?</p>
                  <p className="text-sm text-muted-foreground leading-relaxed">The "Year 2038 problem" affects systems that store Unix timestamps as a 32-bit signed integer — it overflows on 19 January 2038. Modern systems use 64-bit integers, which won't overflow for about 292 billion years.</p>
                </div>
                <div>
                  <p className="text-sm font-semibold text-foreground mb-2">Does the Unix timestamp account for timezones?</p>
                  <p className="text-sm text-muted-foreground leading-relaxed">No. A Unix timestamp always represents a moment in UTC. Timezone conversion happens only at the display layer — the underlying number is the same everywhere in the world at the same instant.</p>
                </div>
                <div>
                  <p className="text-sm font-semibold text-foreground mb-2">Is the timestamp in seconds or milliseconds?</p>
                  <p className="text-sm text-muted-foreground leading-relaxed">The original Unix standard uses seconds. JavaScript's <code className="bg-muted text-foreground/90 px-1 py-0.5 rounded font-mono text-xs">Date.now()</code> returns milliseconds. Many modern APIs (including some databases) use milliseconds. A 13-digit timestamp is almost certainly in milliseconds; a 10-digit one is in seconds.</p>
                </div>
              </div>
          </div>
        </div>
